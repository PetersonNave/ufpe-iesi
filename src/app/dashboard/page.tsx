...
export default function DashboardPage() {
  ...
  const [serverPatients, setServerPatients] = useState<Patient[]>([]);
  const [allPatientsCache, setAllPatientsCache] = useState<Patient[]>([]);
  ...
  const handleClearSearch = () => {
    ...
    setTimeout(() => {
      setLoadingServer(true);
      api
        .get<PatientResponse>(`/api/patients`, {
  ...
  const handleRequestFill = () => {
    ...
    const link = `${window.location.origin}/anamnese?patientId=${
      selectedPatient.id
    ...
    navigator.clipboard
      ...
      .then(() =>
        alert(
          `Link de Anamnese Remota gerado e copiado!\n\nEnvie este link para o paciente:\n${link}`
        )
      )
      .catch(() => alert(`Copie este link manualmente:\n${link}`));
  };
  const getFullAddress = (p: PatientDetail) => {
    const parts = [p.address, p.number, p.neighborhood, p.city, p.state].filter(
      Boolean
    );
                      ...
                      <div className={styles.detailSection}>
                        ...
                        <div className={styles.infoGrid}>
                          ...
                          <p className={styles.fullRow}>
                            ...
                            {getFullAddress(selectedPatient)}
                          </p>
                        </div>
                      </div>
